        ...
        if (user) {
            ...
            if (user.gameState) {
                ...
                for (const key in gameState.ITEMS) {
                    ...
                    if (el)
                        el.textContent = String(gameState.ITEMS[key].amount);
                }
            }
...
function updateAccountDisplay() {
    ...
    if (displayUsername)
        displayUsername.textContent = currentUser.username || 'Not logged in';
    if (displayUserId)
        displayUserId.textContent = currentUser.id ? String(currentUser.id) : '-';
    ...
    if (passwordBtnText)
        passwordBtnText.textContent = 'Show Password';
}
